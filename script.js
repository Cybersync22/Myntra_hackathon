...
const triggerClose = document.querySelectorAll('[trigger-button]');
...
submenu.forEach((menu) => menu.addEventListener('click', function (e) {
    ...
    if (this.closest('.has-child').classList != 'active') {
        thid.closest('.has-child').classList.toggle('active');
    }
}))
...
if (sorter) {
    ...
    sortLi.forEach((item) => item.addEventListener('click', function () {
        ...
        sortLi.querySelector('ul').classList.toggle('show');
    }))
}
...
trigger.forEach((btn) => {
    btn.addEventListener('click', function () {
        ...
        trigger.forEach((s) => s.classList.remove('avtive'));
